...
Solo me vouy a dedicar a hacer la instalacín software(sigueindo la misma [guia](https://github.com/TFG-yisuscc/hailo-rpi5-examples/blob/main/doc/install-raspberry-pi5.md#how-to-set-up-raspberry-pi-5-and-hailo) que la vez anterior)
...
Durante todo este proceso, he obserbado que los tiempos de instalación han disminuido bastante, al menos comparado con lo que recuerdo de la otra vez. -> muy problablementesea por la nueva micro sd(no creo que sea por el firmware peo también podría deberse a eso). Muy probablenmete tendrás que abordarlo cuando elabores la memoria y muy probablemente tengas  que establecer una comparativa.

### Vuelvo a la tarjeta micro sd anterior(la verbatim , marca  taiwanesa por excelencia de los dvd piratas y otros formatos de almacenamiento mu interentes como el cidco hibrido con vinilo):
1. Instalo los driver hailo,
2. Me aseguro de que el firmwre de la eeprom esté actiuualizado
Verifico que a la hora de instalar los drivers la tarjeta vervatim es muchísimo más lenta.
Hago una micro  prueba de rendimiento.
