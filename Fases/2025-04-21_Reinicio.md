...
- Buscar una forma más fiable de medir la carga de la cpu, Actualmente reporta unaos valores de IDLE muy altos para la tarea(en comparación btop reporta en torna un 97% de uso de cpu). Conjeturo que puede deberse a que justo cuando se está haciendo la medición baja la utilización de la cpu al no atenderse el promt(¿al ser multicore no deberia poderse ejecutar en paralelo directamente? -> ver apuntes ac y las referencias de threading).
...
- Que produzca ficheros con los resutados, el  primero contiene las mediciones y el otro parametros relacionados con el promt(el tiempo de inferenciaen 2 timestamps, el promt de entrada y el promt de salida). Algo que creo que puede ser útil es poderevaluar la calidad de las respuestasa posteriori ya que los promts que uso son para evaluar respuestas. 
